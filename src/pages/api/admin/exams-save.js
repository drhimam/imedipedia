 ...
 * Saves the entire exams JSON array to GitHub (src/data/exams.json).
...
export async function POST({ request, locals }) {
  ...
  if (!Array.isArray(exams)) {
    return new Response(JSON.stringify({ error: "exams must be an array of { name, category } objects." }), {
      status: 400, headers: { "Content-Type": "application/json" }
    });
  }
  ...
  for (const e of exams) {
    if (!e.name || typeof e.name !== 'string') {
      return new Response(JSON.stringify({ error: "Each exam must have a 'name' string." }), {
        status: 400, headers: { "Content-Type": "application/json" }
      });
    }
  }

  const filePath = "src/data/exams.json";
  ...
  try {
    ...
    const putBody = {
      message: `Update exam taxonomy (${exams.length} exams)`,
    ...
    if (!putResp.ok) {
      ...
      return new Response(JSON.stringify({ error: `Failed to save exams: ${errMsg}` }), {
        status: 500, headers: { "Content-Type": "application/json" }
      });
    }
    ...
    return new Response(JSON.stringify({
      success: true,
      message: `Exam taxonomy saved (${exams.length} exams). Deploying to Cloudflare...`,
    }), {
